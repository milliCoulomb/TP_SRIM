...
def plot_range(res, fluence, mod):
    ...
    if mod:
        ...
        fig, ax = plt.subplots()
        ax.set_xlabel(r'Depth ($\mu$m)')
        ax.set_ylabel(r'Ba/cm3')
        ax.plot(range.depth / 1e+4, range.ions * fluence, label='Concentration of Ba')
        print("Max of concentration of Ba = "+"{:e}".format(np.max(range.ions * fluence))+" cm3")
        plt.legend()
        plt.tight_layout()
        fig.savefig('range.pdf')
        plt.close(fig)
